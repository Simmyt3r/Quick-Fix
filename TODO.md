...
- [x] Basic monitoring / error tracking — **no new paid service added**, since that's a real decision needing your account/credential, not something I can provision myself. Instead: `/healthz` now actually queries the database (`SELECT 1`) instead of returning a static 200 — a health check that can't detect the DB being unreachable would have missed the exact failure this project already hit once (migrations silently not applied). Documented as standing practice in `ways-of-working.md`: after every production push, check Vercel's runtime logs (`get_runtime_errors`/`get_runtime_logs` via the Vercel MCP tools) for a clean window before considering a deploy verified — this has been done ad hoc after every feature all session, now it's written down as the actual process rather than implicit.
- [ ] A real error-tracking service (Sentry or similar) if error volume ever grows enough that log-checking after each deploy stops being sufficient — needs an account/credential, revisit if needed

## Housekeeping
- [x] `app/static/logo.png` — uploaded and wired into nav, footer, and favicon
- [x] Replace placeholder stats and testimonial on the landing page with real numbers before launch — **N/A, stale item**: the landing page was rebuilt into the current job-status-card splash design in an earlier session, and never carried fabricated stats or a testimonial. Confirmed by reading the current `index.html`.
